...
async function register(req, res) {
  ...
  try {
    ...
    if (existingUser.length > 0) {
      return res
        .status(StatusCodes.BAD_REQUEST)
        .json({ msg:"User already exists.please Enter d/f username or email" });
    }
...
async function login(req, res) {
  ...
  try {
    ...
    if (user.length === 0) {
      return res
        .status(StatusCodes.BAD_REQUEST)
        .json({ msg: "Invalid user credentials.Please try again!" });
    }
    ...
    if (!isSamePassword) {
      return res
        .status(StatusCodes.BAD_REQUEST)
        .json({ msg: "Password is not match.Please try again!" });
    }
